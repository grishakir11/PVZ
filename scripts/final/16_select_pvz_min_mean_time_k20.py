...
# Выбирает K=20 пунктов выдачи так, чтобы минимизировать среднее (взвешенное) время пешком "от дома".
# Без метро. Сравнивает с baseline (первые K из pvz_selected_kmax.csv), который заточен под охват <=10 мин.
...
OUT_PVZ = PROJECT_DIR / "pvz_selected_mean_k20.csv"
OUT_CMP = PROJECT_DIR / "compare_k20_objectives.csv"
...
MAX_CANDIDATES_USED = 900       # кандидатов
DEMAND_WEIGHT_CUM_FRAC = 0.95   # оставить узлы спроса, дающие 95% веса
WALK_SPEED_MPS = 1.3
# =======================
...
def main():
    ...
    total_w_kept = float(demand_wts.sum())
    print(f"  demand_nodes_unique_kept={len(demand_nodes):,} kept_weight_share={total_w_kept/total_w:.3f}")
    ...
    for j, src in enumerate(cn.tolist()):
        dist = nx.single_source_dijkstra_path_length(
            Gu, source=int(src), cutoff=MAX_TIME_S, weight="walk_time_s"
        )
        pairs = []
        g = 0.0
        for node, d in dist.items():
            ...
            dd = float(d)
            pairs.append((idx, dd))
            g += (base_time - dd) * demand_wts[idx]
        cand_pairs.append(pairs)
        init_gain[j] = g
        if (j + 1) % 100 == 0:
            print(f"  precomputed {j+1}/{len(cn)}")

    print("[6/7] Lazy greedy: minimize mean time (maximize time reduction) ...")
    ...
    while len(selected) < K and heap:
        ...
        # если устарело — положим обратно с новым gain
        if gcur < (-neg_g) * 0.98:
    ...
    mean_solution = metrics_from_times(best_time, demand_wts)

    print("[7/7] Compare with baseline (first K from pvz_selected_kmax.csv) ...")
    pvz = pd.read_csv(PVZ_RANKED_CSV, encoding="utf-8-sig").sort_values("sel_rank").reset_index(drop=True)
    pvz_k = pvz.head(K).copy()
    pvz_gdf = gpd.GeoDataFrame(pvz_k, geometry=gpd.points_from_xy(pvz_k["lon"], pvz_k["lat"], crs=WGS84))
    pvz_nodes = nearest_nodes_compat(Gu, X=pvz_gdf.geometry.x.values, Y=pvz_gdf.geometry.y.values)
    pvz_nodes = list(set(int(x) for x in pvz_nodes))

    dist_cov = nx.multi_source_dijkstra_path_length(Gu, sources=pvz_nodes, weight="walk_time_s", cutoff=MAX_TIME_S)
    cov_times = np.array([float(dist_cov.get(int(n), MAX_TIME_S)) for n in demand_nodes], dtype=float)
    cov_solution = metrics_from_times(cov_times, demand_wts)
    ...
    cmp = pd.DataFrame([
        {"method": "baseline_max_coverage_10min", **cov_solution},
        {"method": "greedy_min_mean_time", **mean_solution},
    ])
    cmp.to_csv(OUT_CMP, index=False, encoding="utf-8-sig")

    t1 = perf_counter()
    print("DONE")
    print("PVZ (mean-time) CSV:", OUT_PVZ)
